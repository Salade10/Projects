...
setting_file_path = os.path.join(folder, "settings.txt")

# FR: lire le fichier de parametres
# EN: Read the settings file
with open(setting_file_path, "r") as f:
    lines = f.readlines()
    print(lines)

# EN: Convert whats been read from the settings file into a dict
settings = {k: v for k, v in (line.split("=", 1) for line in lines)}
    ...
    if not word.endswith(".txt"):
        ...
        if isinstance(prefix, str) and prefix.endswith(".txt"):
            try:
                ...
                print(translations[lang]["FileNotFoundError"])
        else:
...
def prowler_shell():
    ...
    while True:
        ...
        shell_parser.add_argument('-settings','--settings', action='store_true')
        ...
        # Add a file full of commands to produce huge wordlists easily
            ...
            # FR Ouvrire le fichier des parametres
            # EN open the settings file
            try:
                folder = os.path.dirname(os.path.abspath(__file__))
                setting_file_path = os.path.join(folder, "settings.txt")
                subprocess.run(f'start "" "{setting_file_path}"', shell=True)
            except subprocess.CalledProcessError:
                print(lang["subprocess_error_CalledProcessError"])
            except FileNotFoundError:
                print(translations[lang]["FileNotFoundError"])
...
parser.add_argument('-w', '--word', required=False, help='The base word or file of words')
...
parser.add_argument('-settings','--settings', action='store_true', help='opens the settings file')
