            ...
            for prefix_item in irp_list:
                for state in states:
                    ...
                    for i, bit in enumerate(state):
                        new_word += word[i].lower() if args.capitalise or not bit else word[i].upper()
            ...
            for suffix_item in irs_list:
                for prefix_item in irp_list:
                    for state in states:
                        new_word = ''
                        for i, bit in enumerate(state):
                            new_word += word[i].lower() if args.capitalise else word[i].upper()
                        full_variant = prefix_item + prefix + new_word + suffix + suffix_item
                        variants.append(full_variant)
...
def run_prowler(args):
    if args.word:
        case_variants = generate_case_permutations(
            args.word, args.prefix, args.suffix,
            args.incrandsuffix, args.incrandprefix
        )
...
# interactive shell
def prowler_shell():
    print("Entering Prowler interactive shell. Type 'exit' to quit.")
    ...
    while True:
        ...
        shell_parser.add_argument('-irp', '--incrandprefix')
        shell_parser.add_argument('-irs', '--incrandsuffix')
